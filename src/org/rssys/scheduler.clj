(ns org.rssys.scheduler
  "Scheduler using Java 19+ Virtual Threads"
  (:import
    (java.io
      Writer)
    (java.text
...
(defn- mk-sched-thread-pool
  ...
  [num-threads]
  (let [t-pool (Executors/newScheduledThreadPool num-threads (thread-factory "scheduled-virtual-pool-"))]
    t-pool))
...
(defn mk-pool
  ...
  PoolInfo record which contains a newly created pool of virtual threads to
  schedule new events for. Pool size defaults to 1M."
  [& {:keys [thread-count]
      :or {thread-count 1000000}}]
  (MutablePool. (atom (mk-pool-info (mk-sched-thread-pool thread-count)))))
...
(defn stop-and-reset-pool!
  ...
  separate virtual thread.  The pool is reset to a fresh new pool preserving
  ...
  (let [pool-atom      (:pool-atom pool)
    ...
    (.submit unbounded-executor ^Callable #(shutdown-pool! old-pool-info strategy))
    old-pool-info))
